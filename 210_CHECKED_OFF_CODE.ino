...
double motorOffset = 1.55;
double turnConstant = 1.5;
float leftMotorSpeed = 100;
double greyInterval = 250000;
...
void stateForward () {
  ...
  if (blackTape(sideLineSensor)) {
    turnCount ++;
    if (turnCount == 1) {
      leftMotorSpeed = (leftMotorSpeed / 1.7) * turnConstant;
      rightMotorSpeed = (leftMotorSpeed * motorOffset) / 1.7;
      turnTimer.reset();
      state = STATE_TURN;
      Serial.print(" state: ");
      Serial.println(state);
...
void adjustSpeedForward () {
  ...
  //    shiftRight();
  //    Serial.println("shift right, and");
  //  }
...
void shiftRight() {
  ...
  leftMotorSpeed = min(motor_baseline_left * 1.15, leftMotorSpeed * speedAdjust);
}

void shiftLeft() {
  leftMotorSpeed = max(1, leftMotorSpeed / speedAdjust);
  rightMotorSpeed = min(motor_baseline_right * 1.15, rightMotorSpeed * speedAdjust);
}
/*---------------------------------------------------------------------------------------------*/
