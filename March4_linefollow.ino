...
#define GREEN_THRESHOLD_LOW    450
#define GREEN_THRESHOLD_HIGH   750
...
//  Serial.println(frontLineSensorLeft);
  ...
  else if (greyTape(frontLineSensorLeft) and greyTape(frontLineSensorRight)) {
    greyTimer.reset();
    state = STATE_GREY;
  }
}
...
void adjustSpeedForwardRearSensors() {
...
void adjustSpeedReverseRearSensors() {
  if (blackTape(frontLineSensorLeft)) {
    shiftLeft();
  } else if (blackTape(frontLineSensorRight)) {
    shiftRight();
